import { DIAGNOSTIC_VERSION } from "./diagnostic-version.js";
...
export function traceMessengerObservation(kind, url, body, blockType = null) {
    ...
    const haystack = `${urlString} ${bodyText}`.toLowerCase();
    const terms = MESSENGER_OBSERVE_TERMS.filter((term) =>
...
function shouldThrottleNearMiss(
    kind,
    urlString,
    blockType,
    terms,
...
function summarizeBody(body, limit = 2000) {
    try {
        ...
        if (body instanceof URLSearchParams)
            return withDecodedText(body.toString(), limit);
...
function makeObservationFlags(haystack) {
    return {
        hasGraphQL:
            haystack.includes("api/graphql") || haystack.includes("doc_id"),
        hasLSRequest:
        ...
        hasReadReceipt:
            ...
            haystack.includes("markthreadasread"),
        hasLegacyTypingEndpoint:
            haystack.includes("ajax/messaging/typ.php") ||
            haystack.includes("ajax/chat/typ.php"),
...
function redactMetadataValue(key, value) {
    ...
    if (
        key === "fb_api_req_friendly_name" &&
        /^[A-Za-z0-9_.$:-]{1,120}$/.test(normalized)
    ) {
        return normalized;
    }
