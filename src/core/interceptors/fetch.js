import {
    sanitizeMessengerNetworkPayload,
    shouldBlock,
    shouldBypassNativeMessageRequestTransport,
...
export function hookFetch() {
    ...
    window.fetch = async function (input, init) {
        ...
        if (
            shouldBypassNativeMessageRequestTransport(earlyBody, url, {
                method,
            })
        ) {
    ...
    if (typeof originalBeacon === "function") {
        navigator.sendBeacon = function (url, data) {
            ...
            if (
                shouldBypassNativeMessageRequestTransport(data, fetchUrl, {
                    method: "POST",
                })
            ) {
