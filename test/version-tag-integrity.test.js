...
const {
    validateVersionTagIntegrity,
} = require("../scripts/validate-version-tag-integrity");
...
function createTaggedFixture() {
    const root = fs.mkdtempSync(
        path.join(os.tmpdir(), "ghostify-version-tag-"),
    );
    git(root, ["init"]);
...
try {
    ...
    assert.strictEqual(
        validateVersionTagIntegrity(unchanged).status,
        "matched",
    );
    ...
    write(
        changedPackage,
        "dist/manifest.json",
        '{"version":"1.2.3","description":"changed"}\n',
    );
    ...
    git(changedFirefoxOverlay, [
        "commit",
        "-m",
        "change Firefox package overlay",
    ]);
    assert.throws(
    ...
    assert.strictEqual(
        validateVersionTagIntegrity(changedPackage).status,
        "untagged",
    );
} finally {
