...
assert(
    fs
        ...
        .includes("DEFAULT_PRIVACY_SETTINGS"),
    "src/config.js should seed mutable MAIN-world settings from shared defaults",
);
assert(
    fs
        .readFileSync("src/messenger_patch.js", "utf8")
        .includes("normalizePrivacySettings"),
...
const popupDefaultsMatch = popupSource.match(
    /const DEFAULT_SETTINGS\s*=\s*({[\s\S]*?});/,
);
assert(
    popupDefaultsMatch,
    "dist/js/popup.js should keep popup defaults until popup source migration",
);
const popupDefaults = vm.runInNewContext(
    `(${popupDefaultsMatch[1]})`,
    Object.create(null),
);
assert.deepStrictEqual(
    JSON.parse(JSON.stringify(popupDefaults)),
    EXPECTED_PRIVACY_SETTINGS,
);
            ...
            local: {
                get(keys, callback) {
                    if (keys.includes(GHOSTIFY_SETTINGS_STORAGE_KEY)) {
                        callback({
                            [GHOSTIFY_SETTINGS_STORAGE_KEY]: storedSettings,
                        });
                        return;
                    }
                    callback({});
...
contentContext.location = { origin: "https://instagram.com" };

vm.runInNewContext(
    fs.readFileSync("dist/js/content.js", "utf8"),
    contentContext,
    {
...
assert(
    ...
    "content script should listen for page settings requests",
);
assert(
    storageChangeListeners.length,
    "content script should listen for settings storage changes",
);
...
const pageSettingsMessages = settingsMessages.filter(
    (message) =>
        message?.type === "GHOSTIFY_SETTINGS_UPDATE" &&
        message.source === "GHOSTIFY_EXTENSION",
);
assert(
    pageSettingsMessages.length >= 2,
    "content script should bridge initial and changed Free settings",
);

for (const message of pageSettingsMessages) {
    assert.deepStrictEqual(
        Object.keys(message.settings),
    ...
    assert.strictEqual(
        Object.prototype.hasOwnProperty.call(message.settings, "accountId"),
        false,
    );
    assert.strictEqual(
        Object.prototype.hasOwnProperty.call(message.settings, "sessionToken"),
        false,
    );
    assert.strictEqual(
        Object.prototype.hasOwnProperty.call(message.settings, "hiddenObject"),
        false,
    );
}
...
vm.runInNewContext(
    fs.readFileSync("dist/background.js", "utf8"),
    backgroundContext,
    {
...
assert.strictEqual(
    installedListeners.length,
    1,
    "background should register its install migration",
);
