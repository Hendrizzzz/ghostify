    ...
    const manifest = firefox
        ? {
              ...
              browser_specific_settings: {
                  gecko: { id: "ghostify@ghostify-extension.vercel.app" },
              },
          }
        : { version: pkg.version };
    const statusData = {
...
async function assertDynamicStatus(target) {
    ...
    assert.strictEqual(
        popup.statusRequests.length,
        1,
        `${target} popup must request the public status feed`,
    );
    const request = popup.statusRequests[0];
    assert.strictEqual(
        request.url,
        "https://ghostify-extension.vercel.app/status.json",
    );
    ...
    assert.strictEqual(
        popup.tooltipElement.textContent,
        "Supported controls passed review",
    );
    assert.strictEqual(
...
main()
    .then(() =>
        console.log(
            `${process.argv[2] || "Firefox and Chromium"} popup status behavior tests passed`,
        ),
    )
    .catch((error) => {
        console.error(error);
