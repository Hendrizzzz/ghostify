...
function copyFixture() {
    const fixtureRoot = fs.mkdtempSync(
        path.join(os.tmpdir(), "ghostify-validate-"),
    );
    ...
    fs.cpSync(path.join(repoRoot, "dist"), path.join(fixtureRoot, "dist"), {
        recursive: true,
    });
    fs.cpSync(path.join(repoRoot, "src"), path.join(fixtureRoot, "src"), {
        recursive: true,
    });
    fs.mkdirSync(path.join(fixtureRoot, "site", "src", "app"), {
        recursive: true,
    });
    fs.copyFileSync(
    ...
    for (const file of ["vercel.json", "vite.config.ts"]) {
        fs.copyFileSync(
            path.join(repoRoot, "site", file),
            path.join(fixtureRoot, "site", file),
        );
    }
    fs.copyFileSync(
...
function runValidator(cwd) {
    return childProcess.spawnSync(
        process.execPath,
        ["scripts/validate-extension-package.js"],
        {
            cwd,
            encoding: "utf8",
        },
...
function proposalTiming(statusJson, hoursAfterLatestStatus = 1) {
    ...
    const generatedAt = new Date(
        Math.max(...baseline) + hoursAfterLatestStatus * 60 * 60 * 1000,
    )
        .toISOString()
        .replace(".000Z", "Z");
    return {
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject a duplicate public status source",
    );
    assert.match(
        result.stderr,
        /must not exist/,
...
withFixture((fixtureRoot) => {
    ...
    const preparedDate = new Date(
        Date.parse(`${statusJson.history[0].date}T00:00:00Z`) +
            24 * 60 * 60 * 1000,
    )
        .toISOString()
        .slice(0, 10);
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject green summary for a different Store build",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject green feature status for a different Store build",
    );
    assert.match(
        result.stderr,
        /publicStatus cannot be verified/,
        result.stderr || result.stdout,
    );
});
...
withFixture((fixtureRoot) => {
    const { statusJson } = readStatusJson(fixtureRoot);
    statusJson.generatedAt = new Date(
        Date.parse(statusJson.release.checkedAt) - 60 * 60 * 1000,
    )
        .toISOString()
        .replace(".000Z", "Z");
    writeStatusJsonPair(fixtureRoot, statusJson);
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject a feed generated before its Store check",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    fs.writeFileSync(
        contentPath,
        source.replace(`version: "${pkg.version}"`, 'version: "0.0.0"'),
    );
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should still parse moved FALLBACK_CONFIG layout",
    );
    assert(
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected manifest permissions",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject duplicate manifest permissions",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected host permissions",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing privacy-policy host permission coverage",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing public status host disclosure",
    );
    assert.match(
        result.stderr,
        /public status feed host disclosure/,
        result.stderr || result.stdout,
    );
});
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing privacy-policy permission coverage",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject stale extra privacy-policy permission entries",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject collection claims that are not scoped to the extension runtime",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing local inspection disclosure",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing fbsbx runtime-narrowing disclosure",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing Chrome Web Store Limited Use disclosure",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    fs.writeFileSync(
        ...
        fs
            .readFileSync(privacyPath, "utf8")
            .replace("GitHub issue forms", "feedback links"),
    );
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing voluntary feedback disclosure",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    fs.writeFileSync(
        ...
        fs
            ...
            .replace(
                "</main>",
                '<a href="https://tally.so/r/test">Feature survey</a></main>',
            ),
    );
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject restored popup survey UI",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    fs.writeFileSync(
        ...
        fs
            .readFileSync(popupPath, "utf8")
            .replace("https://facebook.com/", "https://example.com/"),
    );
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing popup Facebook link",
    );
    assert.match(
        result.stderr,
        new RegExp(
            "dist/popup.html must link Facebook to https://facebook.com/",
        ),
        result.stderr || result.stdout,
    );
});
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing package-version changelog heading",
    );
    assert.match(
        result.stderr,
        new RegExp(
            `CHANGELOG\\.md must include a release heading for ${pkg.version.replace(/\./g, "\\.")}`,
        ),
        result.stderr || result.stdout,
    );
});
...
withFixture((fixtureRoot) => {
    ...
    const statusRoute = vercelConfig.headers.find(
        (route) => route.source === "/status.json",
    );
    statusRoute.headers.find((header) => header.key === "Cache-Control").value =
        "public, max-age=300";
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject a cacheable deployed status feed",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject a cacheable development status feed",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    const vercelPath = path.join(fixtureRoot, "site", "vercel.json");
    const vercelConfig = JSON.parse(fs.readFileSync(vercelPath, "utf8"));
    const siteRoute = vercelConfig.headers.find(
        (route) => route.source === "/(.*)",
    );
    siteRoute.headers = siteRoute.headers.filter(
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing site security headers",
    );
    assert.match(
        result.stderr,
        /site\/vercel\.json must set content-security-policy/,
    );
});
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject weakened site security headers",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject stale public status version",
    );
    assert.match(
        result.stderr,
        new RegExp(
            `site/src/app/statusData\\.json productVersion must be "${pkg.version.replace(/\./g, "\\.")}"`,
        ),
        result.stderr || result.stdout,
    );
});
...
withFixture((fixtureRoot) => {
    ...
    statusJson.history[0].publicStatus = "under_review";
    for (const entry of statusJson.entries) entry.publicStatus = "under_review";
    statusJson.history[0].date = new Date(
        Date.parse(`${statusJson.history[1].date}T00:00:00Z`) - 86_400_000,
    )
        .toISOString()
        .slice(0, 10);
    writeStatusJsonPair(fixtureRoot, statusJson);
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject history that is not newest-first",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject impossible calendar dates",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject auto-green public status policy",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject private messages in verification proof",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject verified public status with manual-pending local evidence",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject community verified status without reviewed community source",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject non-HTTPS public status issue URLs",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    const { statusPath, statusJson } = readStatusJson(fixtureRoot);
    statusJson.entries.push({
        ...statusJson.entries[0],
        id: "duplicate-instagram-hide-seen",
    });
    writeStatusJsonPair(fixtureRoot, statusJson);
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject duplicate platform/feature public status rows",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected raw public status fields",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    const { statusPath, statusJson } = readStatusJson(fixtureRoot);
    statusJson.entries = statusJson.entries.filter(
        (entry) =>
            !(entry.platform === "Facebook" && entry.feature === "Hide Seen"),
    );
    writeStatusJsonPair(fixtureRoot, statusJson);
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject missing platform/feature public status rows",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject optional permissions",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject optional host permissions",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected content-script matches",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected content-script fields",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject unexpected web-accessible resource matches",
    );
    assert.match(
        result.stderr,
...
withFixture((fixtureRoot) => {
    ...
    assert.notStrictEqual(
        result.status,
        0,
        "validator should reject manifest icon dimension mismatches",
    );
    assert.match(
        result.stderr,
