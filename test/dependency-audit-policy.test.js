...
assert(
    evaluateAudit(unrelated, policy, lockfile, reviewDate).blocked.some(
        (reason) => reason.includes("unexpected-package"),
    ),
...
delete changedPath.packages["node_modules/addons-linter"].dependencies[
    "image-size"
];
assert.strictEqual(
...
assert(
    evaluateAudit(changedAuditChain, policy, lockfile, reviewDate).blocked
        .length > 0,
