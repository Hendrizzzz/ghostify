...
const {
    ...
    parseArgs,
    prepareStatusUpdate,
} = require("../scripts/prepare-status-update");
...
const { date: proposalDate, generatedAt: proposalGeneratedAt } =
    proposalContextForStatus(source);
...
function testProposalFixtureFollowsStatusPreparedByWorkflow() {
    const preparedDate = new Date(
        Date.parse(`${source.history[0].date}T00:00:00Z`) + 24 * 60 * 60 * 1000,
    )
        .toISOString()
        .slice(0, 10);
    ...
    assert(
        Date.parse(context.generatedAt) >
            Date.parse(workflowStatus.generatedAt),
    );
    assert.doesNotThrow(() =>
        prepareStatusUpdate(workflowStatus, {
            mode: "verified",
            date: context.date,
...
function testVerifiedProposalTargetsRecordedStoreBuildAcrossVersionMismatch() {
    ...
    mismatchingStatus.entries[0].relatedIssueUrl =
        "https://github.com/Hendrizzzz/Ghostify/issues/123";
    ...
    assert(
        status.entries.every((entry) =>
            entry.reviewRecord.includes("for v2.0.6"),
        ),
    );
    assert(status.entries.every((entry) => entry.relatedIssueUrl === null));
    assert(
        status.history.some((item) => item.title === "Existing review state"),
    );
}
...
function testReportedProposalTurnsSelectedControlAndOverallStatusYellow() {
    ...
    const affected = status.entries.find(
        (entry) => entry.id === "messenger-hide-typing",
    );
    const unaffected = status.entries.find(
        (entry) => entry.id === "instagram-hide-seen",
    );
    ...
    assert.strictEqual(
        unaffected.publicStatus,
        source.entries.find((entry) => entry.id === unaffected.id).publicStatus,
    );
}
...
function testMultipleSameDayStatusUpdatesPreserveNewestFirstOrder() {
    ...
    const confirmed = prepareStatusUpdate(working, {
        ...
        generatedAt: new Date(
            Date.parse(proposalGeneratedAt) + 2 * 60 * 60 * 1000,
        )
            .toISOString()
            .replace(".000Z", "Z"),
        featureIds: ["messenger-hide-typing"],
    ...
    assert.deepStrictEqual(
        confirmed.history
            .slice(0, 3)
            .map((item) => [item.date, item.publicStatus]),
        [
...
function testIdenticalVerificationUpdateDoesNotDuplicateHistory() {
    const freshDate = new Date(
        Date.parse(`${proposalDate}T00:00:00Z`) + 24 * 60 * 60 * 1000,
    )
        .toISOString()
        .slice(0, 10);
    ...
    const secondGeneratedAt = new Date(
        Date.parse(firstGeneratedAt) + 60 * 60 * 1000,
    )
        .toISOString()
        .replace(".000Z", "Z");
...
function testStatusInputsRejectImpossibleDatesAndDuplicateFeatures() {
    ...
    const missingMode = childProcess.spawnSync(
        process.execPath,
        ["scripts/prepare-status-update.js", "--date", "2026-07-12"],
        {
            cwd: repoRoot,
            encoding: "utf8",
        },
...
function testVerificationProposalStateAllowsUpdatesAndYellowRecovery() {
    ...
    assert.deepStrictEqual(
        getVerificationProposalState(mismatchingYellow, { date: proposalDate }),
        {
            targetVersion: "2.0.6",
    ...
    assert.deepStrictEqual(
        getVerificationProposalState(verifiedToday, { date: proposalDate }),
        {
            targetVersion: "2.0.6",
    ...
    const nextDate = new Date(
        ...
        .slice(0, 10);
    assert.strictEqual(
        getVerificationProposalState(verifiedToday, { date: nextDate }).ready,
        true,
    );
    ...
    assert.throws(
        () =>
            getVerificationProposalState(missingPublishedVersion, {
                date: proposalDate,
            }),
...
function testDailyWorkflowIsSingleRefreshableMaintainerApprovalPr() {
    const workflow = fs.readFileSync(
        path.join(repoRoot, ".github", "workflows", "daily-verification.yml"),
        "utf8",
    );
    ...
    assert(
        workflow.includes(
            "main changed while this proposal was being validated",
        ),
    );
