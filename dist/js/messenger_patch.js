(() => {
  ...
  (function() {
    ...
    registerSourceReplacement("MAWSecureTypingState", (source) => {
      var _a;
      if ((_a = window.__GHOSTIFY_SETTINGS__) == null ? void 0 : _a.msgTyping) {
    ...
    function shouldProcessModule(moduleName) {
      return Object.keys(sourceReplacements).some((p) => moduleName.includes(p)) || Object.keys(exportCallbacks).some((p) => moduleName.includes(p));
    }
          ...
          apply: (fn, thisArg, args) => {
            ...
            if (typeof moduleName === "string" && !processedModules.has(moduleName)) {
              if (shouldProcessModule(moduleName)) {
                console.log("\u{1F47B} [MODULE] Intercepting: " + moduleName);
