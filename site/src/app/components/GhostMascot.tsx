...
const THROW_PHRASES = ['nice throw.', 'good toss.', 'whee. local-only flight.', 'that had range.'];

export function GhostMascot() {
  const [message, setMessage] = useState<string | null>(null);
  const [isFlying, setIsFlying] = useState(false);
  const [typingMotion, setTypingMotion] = useState<'idle' | 'burst' | 'active' | 'settling'>(
    'idle',
  );
  ...
  const safeMascotPos = useCallback(() => {
    ...
    const clamp = (x: number, y: number) => ({
      x: Math.max(12, Math.min(vw - GHOST_W - 12, x)),
      y: Math.max(120, Math.min(vh - GHOST_H - 12, y)),
    });

    // Candidate perches, best first. Corners and gutters beat mid-content:
    // screenshots, toggle rows, and annotation pills all live mid-column.
    const candidates =
      vw <= 900
        ? [
            { x: vw - GHOST_W - 14, y: vh - GHOST_H - 18 },
            { x: 12, y: vh - GHOST_H - 18 },
            { x: vw - GHOST_W - 14, y: Math.max(140, vh * 0.62) },
            { x: 12, y: Math.max(140, vh * 0.62) },
          ]
        : [
            { x: vw - GHOST_W - 24, y: vh - GHOST_H - 20 },
            { x: 20, y: vh - GHOST_H - 20 },
            { x: vw - GHOST_W - 88, y: Math.max(112, vh * 0.22) },
            { x: 20, y: Math.max(112, vh * 0.3) },
          ];

    // Anything that counts as "content the ghost must not cover". Broad on
    // purpose — an over-match just means the ghost prefers a bare gutter.
    const CONTENT_SELECTOR =
      'img,svg,video,button,a,p,h1,h2,h3,h4,li,input,label,[data-hero-cursor],' +
      '[class*="card"],[class*="toggle"],[class*="switch"],[class*="browser"],' +
      '[class*="shot"],[class*="media"],[class*="panel"],[class*="sheet"],' +
      '[class*="annotation"],[class*="callout"],[class*="pill"],[class*="bubble"]';

    const blockedAt = (px: number, py: number) => {
      const probe = (x: number, y: number) => {
        const el = document.elementFromPoint(x, y);
        return el ? el.closest(CONTENT_SELECTOR) !== null : true;
      };
      // Probe the ghost body plus the full speech-bubble band above it
      // (the bubble tops out ~50px up, so sweep the strip).
      return (
        probe(px + GHOST_W / 2, py + GHOST_H / 2) ||
        probe(px + GHOST_W / 2, Math.max(8, py - 20)) ||
        probe(px + GHOST_W / 2, Math.max(8, py - 58))
      );
    };

    // Hide the ghost while probing so elementFromPoint can't hit it.
    const ghost = ghostRef.current;
    const restoreVisibility = ghost && ghost.style.visibility !== 'hidden';
    if (restoreVisibility && ghost) ghost.style.visibility = 'hidden';

    let pick: { x: number; y: number } | null = null;
    try {
      for (const candidate of candidates) {
        const spot = clamp(candidate.x, candidate.y);
        if (!blockedAt(spot.x, spot.y)) {
          pick = spot;
          break;
        }
      }

      // Everything preferred is covered (e.g. a full-bleed phone screenshot):
      // sweep both gutters vertically for any clear band before giving up.
      if (!pick) {
        const xs = vw <= 900 ? [vw - GHOST_W - 14, 12] : [vw - GHOST_W - 24, 20];
        const minY = 120;
        const maxY = vh - GHOST_H - 12;
        const steps = 9;
        sweep: for (let i = 0; i < steps; i += 1) {
          for (const x of xs) {
            const spot = clamp(x, minY + ((maxY - minY) * i) / (steps - 1));
            if (!blockedAt(spot.x, spot.y)) {
              pick = spot;
              break sweep;
            }
      ...
      if (restoreVisibility && ghost) ghost.style.visibility = '';
    }

    return pick ?? clamp(vw - GHOST_W - 24, vh - GHOST_H - 20);
  }, []);
  ...
  const applyVisualPos = useCallback(
    (x: number, y: number) => {
  ...
  const glideToHeroCallout = useCallback(
    (type: string) => {
      const parked = window.innerWidth <= 900 ? safeMascotPos() : null;
      ...
      const useDemoSpot =
        ...
        demoRect.height > 0 &&
        demoRect.bottom > 0 &&
        demoRect.top < window.innerHeight;
    ...
    if (!isDragging.current) {
      if (isHoldingCenter.current) {
        ...
        if (speed > MIN_SPEED) {
          ...
          // Idle drift — very slow, and repainted at a third of the frame
          // rate: a parked ghost doesn't need 60 transform writes a second.
          idleFrame.current = (idleFrame.current + 1) % 3;
          if (idleFrame.current === 0) {
            idleT.current += 0.021;
            const dx = Math.sin(idleT.current * 0.22 + idlePhase.current) * 0.24;
            const dy = Math.cos(idleT.current * 0.16 + idlePhase.current + 1) * 0.15;
            ...
            // Small screens: a ghost parked over copy is in the way. Let it
            // linger ~7s where it was left, then glide back to its corner.
            if (window.innerWidth <= 900) {
              const safe = safeMascotPos();
              if (
                reparkAt.current === 0 &&
                Math.hypot(pos.current.x - safe.x, pos.current.y - safe.y) > 24
              ) {
                reparkAt.current = Date.now() + 7000;
              } else if (reparkAt.current !== 0 && Date.now() >= reparkAt.current) {
                reparkAt.current = 0;
                centerTarget.current = safe;
                isHoldingCenter.current = true;
                holdCenterUntil.current = Date.now() + 4000;
              }
            }
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if ((type === 'chat-open' || type === 'story-view') && window.innerWidth <= 900) {
        stopTypingMotion();
        parkAtSafeEdge();
  ...
  return (
    <>
              ...
              {!bubbleBelow && (
                <div
                  style={{
                    ...
                    borderTop: '5px solid rgba(18,18,19,0.94)',
                  }}
                />
              )}
              {bubbleBelow && (
                <div
                  style={{
                    ...
                    borderLeft: '5px solid transparent',
                    borderRight: '5px solid transparent',
                    borderBottom: '5px solid rgba(18,18,19,0.94)',
                  }}
                />
              )}
            </div>
      ...
      >
        ...
        >
          <GhostMark size={GHOST_W} bodyColor="#0f0f0d" eyeColor="#ffffff" />
        </div>
      </div>
      <style>{`
        .ghost-mascot-shell svg {
          width: var(--ghost-mascot-size, 64px);
          height: var(--ghost-mascot-size, 64px);
        }
        @media (prefers-reduced-motion: reduce) {
          .ghost-mascot-shell > div {
