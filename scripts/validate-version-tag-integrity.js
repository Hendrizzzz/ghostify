...
function validateVersionTagIntegrity(repoRoot = process.cwd()) {
    ...
    if (tagLookup.status === 1)
        return { status: "untagged", tag, changedPaths: [] };
    if (tagLookup.status !== 0)
        failFromGit(tagLookup, `Could not inspect ${tag}.`);

    const comparison = git(repoRoot, [
        "diff",
        "--quiet",
        tag,
        "--",
    ...
    if (comparison.status === 0)
        return { status: "matched", tag, changedPaths: [] };
    if (comparison.status !== 1)
        failFromGit(
            comparison,
            `Could not compare package inputs with ${tag}.`,
        );
    ...
    if (changed.status !== 0)
        failFromGit(
            changed,
            `Could not list package input changes since ${tag}.`,
        );
...
function main() {
    ...
    if (result.status === "untagged") {
        console.log(
            `${result.tag} is not tagged; package identity is available for the current development version`,
        );
    } else {
