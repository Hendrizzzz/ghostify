...
function validateExceptionContext(exception, lockfile, now = new Date()) {
    if (exception.scope !== 'development-only') {
        return { valid: false, reason: `${exception.id}: scope must be development-only` };
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(exception.expiresOn || '')) {
        return { valid: false, reason: `${exception.id}: expiresOn must use YYYY-MM-DD` };
    }
    const expiresAt = new Date(`${exception.expiresOn}T23:59:59.999Z`);
    if (Number.isNaN(expiresAt.getTime()) || now > expiresAt) {
        return { valid: false, reason: `${exception.id}: exception expired on ${exception.expiresOn}` };
    }

    const dependencyPath = exception.dependencyPath || [];
    if (dependencyPath.length < 2 || dependencyPath.at(-1) !== exception.package) {
        return { valid: false, reason: `${exception.id}: dependencyPath must end at ${exception.package}` };
    }

    const packages = lockfile.packages || {};
    const root = packages[''] || {};
    if (!Object.hasOwn(root.devDependencies || {}, dependencyPath[0])) {
        return { valid: false, reason: `${exception.id}: ${dependencyPath[0]} is not a direct development dependency` };
    }
    if (Object.hasOwn(root.dependencies || {}, dependencyPath[0])) {
        return { valid: false, reason: `${exception.id}: ${dependencyPath[0]} is also a runtime dependency` };
    }
    ...
    for (let index = 0; index < dependencyPath.length; index += 1) {
        ...
        if (!node || node.dev !== true) {
            return { valid: false, reason: `${exception.id}: ${name} is missing or is not development-only` };
        }
        const next = dependencyPath[index + 1];
        if (next && !Object.hasOwn(node.dependencies || {}, next)) {
            return { valid: false, reason: `${exception.id}: ${name} does not depend on ${next}` };
        }
    }

    const target = packages[exception.node];
    if (!target || target.version !== exception.version || target.dev !== true) {
        return {
            valid: false,
...
function exceptionAllows(name, vulnerabilities, exception, visiting = new Set()) {
    ...
    if (!Array.isArray(vulnerability.nodes) || vulnerability.nodes.some(node => node !== expectedNode)) return false;

    const nextVisiting = new Set(visiting).add(name);
    const allowedAdvisories = new Set(exception.advisories.map(value => value.toUpperCase()));
    return vulnerability.via.every(via => {
        if (typeof via === 'string') {
            const expectedNext = exception.dependencyPath[pathIndex + 1];
            return via === expectedNext && exceptionAllows(via, vulnerabilities, exception, nextVisiting);
        }
        const id = advisoryId(via);
        const isTarget = pathIndex === exception.dependencyPath.length - 1 && name === exception.package;
...
function evaluateAudit(report, policy, lockfile, now = new Date()) {
    if (report.auditReportVersion !== 2 || typeof report.vulnerabilities !== 'object') {
        return { allowed: [], blocked: ['npm audit returned an unsupported report format'] };
    }
    if (policy.schemaVersion !== 1 || !Array.isArray(policy.exceptions)) {
        return { allowed: [], blocked: ['dependency audit exception policy is invalid'] };
    }
    ...
    for (const name of blocking) {
        const match = contexts.find(({ exception, context }) =>
            context.valid && exceptionAllows(name, report.vulnerabilities, exception)
        );
        if (match) {
            allowed.push({ name, exception: match.exception });
        } else {
            const relevantInvalid = contexts.find(({ exception }) => exception.dependencyPath?.includes(name));
            blocked.push(relevantInvalid?.context.reason || `${name}: unreviewed high or critical vulnerability`);
        }
    }
...
function runAudit(rootDir = path.resolve(__dirname, '..')) {
    ...
    if (!npmCli) {
        console.error('npm_execpath is unavailable; run this check through npm run audit:high.');
        return 2;
    }
    ...
    const policy = JSON.parse(fs.readFileSync(path.join(rootDir, '.github', 'dependency-audit-exceptions.json'), 'utf8'));
    ...
    const exceptions = [...new Map(decision.allowed.map(item => [item.exception.id, item.exception])).values()];
    console.warn('Dependency audit passed with reviewed, temporary development-only exceptions:');
    ...
    console.warn('All other high and critical dependency findings remain blocking.');
    return 0;
}
