...
const YELLOW_MODES = {
    reported: {
        ...
        message: (labels) =>
            `We’re looking into reports affecting ${labels}. We’ll share another update as we learn more.`,
    },
    "in-progress": {
        ...
        message: (labels) =>
            `We’ve identified an issue affecting ${labels} and are working on a fix.`,
    },
    "known-issue": {
        ...
        message: (labels) =>
            `Some users may have trouble with ${labels}. We’re working on it.`,
    },
};
...
function assertGeneratedAt(generatedAt, date, releaseCheckedAt) {
    ...
    if (parsed.getTime() < Date.parse(isoAtUtcStart(date))) {
        throw new Error(
            "generated-at timestamp must not predate the status date.",
        );
    }
    if (releaseCheckedAt && parsed.getTime() < Date.parse(releaseCheckedAt)) {
        throw new Error(
            "generated-at timestamp must not predate the recorded Store check.",
        );
    }
}
...
function prependHistory(status, item) {
    ...
    status.history = [
        item,
        ...history.filter((existing) => !isIdenticalEntry(existing)),
    ];
}
...
function assertNotBackdated(status, date) {
    ...
    if (
        latestDate &&
        Date.parse(isoAtUtcStart(date)) < Date.parse(isoAtUtcStart(latestDate))
    ) {
        throw new Error(
            `Status date ${date} must not predate latest history date ${latestDate}.`,
        );
    }
}
...
function assertKnownFeatureIds(status, featureIds, mode) {
    if (!featureIds?.length)
        throw new Error(`${mode} updates require at least one feature id.`);
    if (new Set(featureIds).size !== featureIds.length)
        throw new Error("Feature ids must not contain duplicates.");
...
function assertIssueUrl(issueUrl) {
    if (
        issueUrl &&
        !/^https:\/\/github\.com\/Hendrizzzz\/Ghostify\/issues\/\d+(?:[/?#].*)?$/.test(
            issueUrl,
        )
    ) {
        throw new Error(
            "Issue URL must point to a Ghostify GitHub issue over HTTPS.",
        );
    }
}
...
function getPublishedVersion(status) {
    ...
    if (typeof publishedVersion !== "string" || !publishedVersion.trim()) {
        throw new Error(
            "A recorded Chrome Web Store published version is required for verification.",
        );
    }
...
function prepareVerifiedStatus(input, { date, generatedAt }) {
    ...
    status.entries = status.entries.map((entry) => {
        ...
        if (!qaId)
            throw new Error(`Missing QA fixture mapping for ${entry.id}`);
        return {
            ...entry,
            publicStatus: "maintainer_verified",
...
function prepareYellowStatus(
    input,
    { mode, date, generatedAt, featureIds, issueUrl = null, note },
) {
    const status = structuredClone(input);
    ...
    status.entries = status.entries.map((entry) => {
        ...
        return {
            ...
            notes:
                note ||
                `${modeConfig.label} for ${entry.platform} ${entry.feature} as of ${longDate}.`,
        };
    });
...
function prepareStatusUpdate(input, options) {
    if (options.mode === "verified")
        return prepareVerifiedStatus(input, options);
...
function run(argv = process.argv.slice(2)) {
    ...
    if (!args.mode)
        throw new Error(
            "--mode is required; green verification must be explicit.",
        );
    const now = new Date();
    const date = args.date || now.toISOString().slice(0, 10);
    const generatedAt =
        args["generated-at"] || now.toISOString().replace(/\.\d{3}Z$/, "Z");
