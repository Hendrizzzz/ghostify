...
function assertDatedReleaseHeading(changelog, version) {
    ...
    if (!match) {
        throw new Error(
            `CHANGELOG.md release ${version} must have a dated YYYY-MM-DD heading before publication.`,
        );
    }
    ...
    if (
        ...
        throw new Error(
            `CHANGELOG.md release ${version} has an invalid publication date.`,
        );
    }
...
function main(argv) {
    ...
    if (!version || argv.length > 2) {
        throw new Error(
            "Usage: node scripts/extract-release-notes.js <version> [changelog-path]",
        );
    }
