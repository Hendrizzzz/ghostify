...
} = require("./prepare-firefox-extension");
...
const chromiumManifest = JSON.parse(
    fs.readFileSync(path.join(repoRoot, "dist", "manifest.json"), "utf8"),
);
const firefoxManifest = createFirefoxManifest();
const popupJs = fs.readFileSync(
    path.join(repoRoot, "dist", "js", "popup.js"),
    "utf8",
);
const firefoxReleaseChecklist = fs.readFileSync(
    path.join(repoRoot, "docs", "FIREFOX_RELEASE_CHECKLIST.md"),
    "utf8",
);
const firefoxSubmissionPacket = fs.readFileSync(
    path.join(repoRoot, "docs", "FIREFOX_AMO_SUBMISSION.md"),
    "utf8",
);
const browserDistribution = fs.readFileSync(
    path.join(repoRoot, "docs", "BROWSER_DISTRIBUTION.md"),
    "utf8",
);
...
if (firefoxManifest.browser_specific_settings?.gecko?.id !== expectedId) {
    fail(
        `Firefox manifest must preserve the permanent add-on ID ${expectedId}`,
    );
}
if (
    ...
    fail(
        "Firefox strict_min_version must remain 140.0 or be reviewed with the compatibility contract",
    );
}
if (
    ...
    fail(
        "Firefox for Android parser compatibility must remain at 142.0 or later",
    );
}
if (
    JSON.stringify(
    ...
    fail(
        "Firefox package must declare no collection or transmission of user data",
    );
}
...
if (!chromiumManifest.host_permissions.includes(statusHost)) {
    fail(
        "Chromium package must retain the Ghostify status-feed host permission",
    );
}
if (
    JSON.stringify(firefoxManifest.host_permissions) !==
...
if (
    !popupJs.includes("new XMLHttpRequest()") ||
    popupJs.includes("fetch(PUBLIC_STATUS_FEED_URL")
) {
    fail(
        "Popup public-status request must use privileged XMLHttpRequest rather than Fetch",
    );
}
...
for (const [file, contents] of androidPublicationDocs) {
    for (const phrase of obsoleteDesktopOnlyPhrases) {
        if (phrase.test(contents))
            fail(
                `${file} contradicts the Firefox Android manifest declaration`,
            );
    }
}
if (
    !firefoxReleaseChecklist.includes(
        "Firefox for Android minimum version: `142.0`",
    )
) {
    fail(
        "Firefox release checklist must record the Firefox for Android 142.0 minimum",
    );
}
if (
    !firefoxSubmissionPacket.includes("Firefox desktop and Firefox for Android")
) {
    fail(
        "Firefox AMO submission packet must select desktop and Android compatibility",
    );
}
...
for (const key of comparableKeys) {
    if (
        JSON.stringify(firefoxManifest[key]) !==
        JSON.stringify(chromiumManifest[key])
    ) {
        fail(
            `Firefox manifest unexpectedly diverges from Chromium field: ${key}`,
        );
    }
}
if (
    JSON.stringify(firefoxManifest.icons) !==
    JSON.stringify({
        16: "icons/icon16.png",
    ...
    fail(
        "Firefox manifest must package only icons whose declared dimensions match their files",
    );
}
